    ...
    public static async Task<Program.CliExitCode> RunAsync(string[] args)
    {
        try
        {
            ...
            if (commandOptions.Command == "restore")
            {
                ...
                SockseekLog.Info($"Restored {restored.DatabasePath} ({restored.SizeBytes} bytes); integrity={restored.Integrity.Result}");
            ...
            if (commandOptions.Command == "migrate")
            {
                ...
                SockseekLog.Info($"Migrated {databasePath}; schema={initialized.SchemaVersion}");
                return Program.CliExitCode.Success;
            }
            ...
            if (commandOptions.Command == "integrity")
            {
                ...
                if (integrity.IsHealthy)
                    SockseekLog.Info($"Database integrity check passed: {integrity.Result}");
                else
                    SockseekLog.Error($"Database integrity check failed: {integrity.Result}");
            ...
            if (commandOptions.Command == "backup")
            {
                ...
                SockseekLog.Info($"Backed up {backup.BackupPath} ({backup.SizeBytes} bytes); integrity={backup.Integrity.Result}");
                return Program.CliExitCode.Success;
            }
        ...
        {
            SockseekLog.Error($"Database command failed: {SockseekLog.ExceptionSummary(ex)}");
