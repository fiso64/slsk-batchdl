...
<article
  class="item-card result-card track-item-card"
  class:locked
  class:selected
  class:preferred
  class:transfer-card={Boolean(transfer)}
>
  <svelte:element this={selectable ? 'label' : 'div'} class:clickable={selectable} class="track-item-main track-result-row">
    {#if selectable}
      <input
        type="checkbox"
        checked={selected}
  ...
  {#if transfer}
    <div class={`transfer-card-footer ${transfer.tone ?? ''}`}>
