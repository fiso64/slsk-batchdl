...
export type ExtractSourceType = 'spotify' | 'youtube' | 'bandcamp' | 'musicbrainz' | 'soulseek' | 'csv' | 'list' | 'string';
...
export function createInitialAutomaticJobs(scenario: 'normal' | 'busy' | 'loading' | 'empty' | 'offline' | 'stress' = 'normal'): AutomaticJobRecord[] {
  ...
  const records: AutomaticJobRecord[] = [
    ...
    },
    {
      ...
      payload: { sourceType: 'csv', input: 'artifact:library-import.csv', resultJobId: csvListId, autoStartResult: true },
    },
    {
      id: csvListId,
    ...
    },
    {
      ...
      payload: { sourceType: 'list', input: 'artifact:favorites.list', resultJobId: jobId(71), autoStartResult: true },
    },
    {
...
function selectedPreviewTree(node: JobPreviewNode, selectedLeaves: Set<string>): JobPreviewNode | null {
  ...
  const children = node.children.map((child) => selectedPreviewTree(child, selectedLeaves)).filter((child): child is JobPreviewNode => child !== null);
  if (!children.length) return null;
  return { ...node, children };
}

function runtimeJobFromPreview(node: JobPreviewNode, workflow: string, parentJobId: string | null, createdAtUtc: string, index: number): AutomaticJobRecord[] {
  const id = prototypeUuid(0x51000000, createdJobSequence++);
  const base = { id, workflowId: workflow, parentJobId, title: node.title, subtitle: node.detail, status: 'pending' as const, createdAtUtc, when: 'just now', lifetime: 'live' as const };

  if (node.kind === 'extract') {
    const resultNode = node.children[0] ?? null;
    const resultRecords = resultNode ? runtimeJobFromPreview(resultNode, workflow, null, createdAtUtc, index + 1) : [];
    const resultId = resultRecords[0]?.id ?? null;
    const record: ExtractJobRecord = { ...base, kind: 'extract', payload: { sourceType: node.sourceType ?? 'string', input: node.detail ?? node.title, resultJobId: resultId, autoStartResult: true } };
    return [record, ...resultRecords];
  }

  if (node.kind === 'job-list') {
    const record: JobListRecord = { ...base, kind: 'job-list', progress: { completed: 0, total: node.children.length }, payload: { name: node.title, childCount: node.children.length, succeeded: 0, failed: 0 } };
    const children = node.children.flatMap((child, childIndex) => runtimeJobFromPreview(child, workflow, id, createdAtUtc, index + childIndex + 1));
  ...
  if (node.kind === 'remote-directory') {
    const record: RemoteDirectoryJobRecord = { ...base, kind: 'remote-directory', payload: { username: 'nightshift', folderPath: node.title.split(' — ').at(-1) ?? node.title, files: [] } };
    return [record];
  }
  ...
  if (node.kind === 'retrieve-folder') {
    const record: RetrieveFolderJobRecord = { ...base, kind: 'retrieve-folder', payload: { username: 'nightshift', folderPath: node.title, newFilesFoundCount: 0, outcome: 'retrieving' } };
    return [record];
  }
...
export function commitPreview(plan: JobPreviewPlan, selectedLeaves: Set<string>): { records: AutomaticJobRecord[]; rootId: string | null } {
  const selectedRoots = plan.roots.map((root) => selectedPreviewTree(root, selectedLeaves)).filter((root): root is JobPreviewNode => root !== null);
  if (!selectedRoots.length) return { records: [], rootId: null };
  const workflow = prototypeUuid(0x51010000, createdJobSequence++);
  const createdAtUtc = new Date().toISOString();
  const records = selectedRoots.flatMap((root, index) => runtimeJobFromPreview(root, workflow, null, createdAtUtc, index));
