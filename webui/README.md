...
**Wishlists** are persisted, scheduled job-list definitions owned by **Jobs**, not an eighth shell destination and not a new runtime Job kind. The Jobs heading keeps **New wishlist** immediately beside **New job**; when at least one wishlist exists, a compact Wishlists section appears above Job history. Opening a wishlist stays inside the Jobs navigation context and exposes its enabled **Daily / Weekly / Monthly / every N minutes-or-hours** schedule, manual **Run now** (or **Cancel** while its current workflow is active), ordered mixed job definitions, and global **Import / Filtering / Ranking / Download** defaults. Disabling the schedule is the manual-only state rather than a separate cadence. Wishlist items are added and edited through the same New Job composer in a definition mode rather than through a second source-picker implementation. Unchanged item settings use the wishlist defaults; item-specific configuration is conceptually a sparse override so later default changes still flow to fields the item did not override. Each saved item row contains only definition metadata/editing controls; when that item has executed, its latest ordinary runtime job is rendered immediately beneath it with the same shared compact Job row used for Job List children and other nested job navigation. Recurring executions stay out of the ordinary root Job-history list, but they do **not** create a parallel Wishlist execution-history model: each run is an ordinary persisted Workflow/Job execution associated back to the wishlist/run/item through stable provenance, and any bounded Wishlist-run listing is an index over those workflows. Detail-page lifecycle controls are deliberately consistent across Wishlists, Searches, and automatic jobs: active resources expose the Wishlist-style **Cancel** action and do not simultaneously show Delete/Remove; terminal resources use the compact trash action, while an idle Wishlist also keeps its separate **Run now** action.
...
The prototype now opens on a Dashboard tab. Its history range control is intentionally dashboard-wide rather than chart-only: switching between 24h, 7d, 30d, 90d, 1y, and All updates all four historical KPI cards (Downloaded, Uploaded, Share ratio, and **Distinct peers**), the transfer activity chart, and Downloads/Uploads/Content/Errors ranking data together. Distinct peers means unique remote usernames observed across download and upload transfers in the selected range, counted once across both directions. When persisted Wishlists exist, the ranking panel shares its row with a **Wishlists** panel at an intentional desktop **4/7 rankings : 3/7 Wishlists** split. The panel uses the same compact list-row presentation regardless of whether there is one Wishlist or several, avoiding count-specific layout changes. Hovering an entry’s clock affordance reveals the relevant **Run** or **Cancel** action, while clicking the entry itself opens the Wishlist in Jobs; with no Wishlists, rankings retain the full row width. Current download/upload rates and active-transfer count live together in the bottom **Transfer rates** panel instead of competing with historical totals at the top. The historical Downloaded/Uploaded/Share ratio cards retain compact range-dependent bar strips for quick visual texture without replacing the main transfer-activity chart.
