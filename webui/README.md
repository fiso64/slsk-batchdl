...
Job detail uses one shared shell with type-specific bodies rather than independent page layouts. Song and Album center the selected File/Folder presentation used elsewhere; Job List uses consistent child rows whose primary line is the job identity, whose secondary line carries type/time context, and whose right-side metadata keeps statistics and lifecycle state together. For resolved Song/Album and exact Remote File/Remote Directory children, that same secondary line also keeps the remote username and path visible without increasing row height; the username remains a normal quiet clickable user link and the path truncates before forcing the row taller. Measurable active progress is the only extra row. Active/failed Extract jobs show their source/input, but once extraction has completed the UI silently presents its semantic `ResultJobId` in place of the wrapper so a CSV/Spotify import opens directly to the useful result. Nested child navigation is one level at a time: the Back control returns to the semantic parent, and the single-line ancestry/type kicker exposes the same hierarchy without moving the page title between job kinds. `ParentJobId` remains execution hierarchy while Extract-result flattening is presentation-only. Manual `AwaitingSelection` behavior is not surfaced in this first automatic-job pass.

Search result refinement is deliberately compact: text filtering, sorting, and a **Filtering** button share one row. Applied condition pills render only when needed and reserve no empty row when a search has no hard filters. The Filtering button opens the same `SearchConfigPanel` component used by the global search rather than a second ad-hoc UI; its Ranking tab is available there as well. Text, Filtering, Ranking, and Sort changes each request a new daemon-owned projection over the complete retained result set and reset result pagination; they are not client-side operations over pages already received. The mock adapter responds synchronously only because the prototype is unwired. Search-result keyboard navigation treats each top-level projection unit as one result: Up/Down moves a visibly distinct current result, Space toggles that result's selection, and `R` retrieves every fully selected folder result where folder retrieval applies. Album/File Search navigation never descends into nested child-file rows. Moving the current item gives it programmatic focus so the next Tab continues through that item's controls; conversely, tabbing into a result makes that result current, and Up/Down continue to work from non-text controls such as checkboxes. Down at the last loaded result automatically requests the next result page when one exists and advances into it; landing on the first result reveals the top of the view, including its heading/refinement controls. The shared selection/download toolbar owns Enter, so selected downloads can be submitted without a current cursor anywhere that toolbar appears; its button shows a compact **⏎** keycap hint. Focused selection checkboxes do not suppress that Enter shortcut (Space remains their native toggle), while text editing, native Enter activation on other controls, and modal/popover ownership still win over shortcuts. On the Jobs history overview, Up/Down move within a lane, automatically extending that lane at its loaded boundary, and Left/Right cross between the Searches and Other jobs lanes at the nearest row. Landing on the top row similarly restores the full Jobs heading area.
